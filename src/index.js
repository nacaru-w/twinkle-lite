...
const loadMoreBits = () => {
	return new Promise((resolve, reject) => {
		mw.loader.load('https://en.wikipedia.org/w/index.php?title=MediaWiki:Gadget-morebits.js&action=raw&ctype=text/javascript', 'text/javascript', resolve, reject);
	});
}

const loadMoreBitsCSS = () => {
	return new Promise((resolve, reject) => {
		mw.loader.load('https://en.wikipedia.org/w/index.php?title=MediaWiki:Gadget-morebits.css&action=raw&ctype=text/css', 'text/css', resolve, reject)
	});
}
...
const loadTwinkleLite = async () => {
	try {
		await loadMoreBits();
		await loadMoreBitsCSS();
		initializeTwinkleLite();
	} catch (error) {
		console.log("Failed to load Twinkle Lite:", error)
	}
}
