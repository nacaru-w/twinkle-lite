...
if (!window.IS_TWINKLE_LITE_LOADED) {
	...
	const initializeTwinkleLite = async () => {
		...
		if (isCurrentUserSysop && currentNamespace == 4 && currentPageName.startsWith('Wikipedia:Consultas_de_borrado/')) {
			if (await isDeletionRequestOpen(currentPageName)) {
		...
		if (isCurrentUserSysop && currentSkin !== 'minerva' && currentNamespace == 4 && isMainPage && currentPageName.startsWith('Wikipedia:Tablón_de_anuncios_de_los_bibliotecarios/') && currentPageName.endsWith('/Actual')) {
